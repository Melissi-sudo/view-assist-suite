...
local function resolveStageOne()
	...
	return stages:FindFirstChild("1") or stages:FindFirstChild(1)
end
...
local function animateButton(button, baseColor, hoverColor, strokeColor)
	...
	local function setHover(on)
		TweenService:Create(button, TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			BackgroundColor3 = on and hoverColor or baseColor
		}):Play()
		TweenService:Create(stroke, TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Color = on and (strokeColor or Color3.fromRGB(235, 70, 70)) or Color3.fromRGB(110, 20, 20),
			Transparency = on and 0.06 or 0.28
		}):Play()
	end
...
local function createCard(parent, height, titleText, descText)
	...
	grad.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(34, 18, 18)),
	...
	title.TextSize = 15
	title.TextColor3 = Color3.fromRGB(255, 236, 236)
	title.Parent = card

	local desc = Instance.new("TextLabel")
	desc.BackgroundTransparency = 1
	desc.Position = UDim2.new(0, 0, 0, 24)
	desc.Size = UDim2.new(1, 0, 0, math.max(18, height - 48))
...
local homeHero = createCard(homePage, 108, "Control Hub", "Polished teleport control with auto-return protection, smooth motion, and an adjustable custom target part.")
...
homeStats.Size = UDim2.new(1, 0, 0, 72)
...
createLabel(toggleCard, "Auto-return guard", UDim2.new(1, -80, 0, 18), Color3.fromRGB(255, 235, 235), true).TextSize = 14
...
settingsCard.Size = UDim2.new(1, 0, 0, 250)
...
createLabel(settingsCard, "Teleport Settings", UDim2.new(1, 0, 0, 18), Color3.fromRGB(255, 235, 235), true).TextSize = 15
...
inputHolder.Size = UDim2.new(1, 0, 0, 150)
...
createLabel(inputHolder, "Target part name", UDim2.new(1, 0, 0, 16), Color3.fromRGB(255, 235, 235), true)
local targetBox = createInput(inputHolder, "Win Part", config.TargetPartName)

createLabel(inputHolder, "Auto-return distance", UDim2.new(1, 0, 0, 16), Color3.fromRGB(255, 235, 235), true)
local distanceBox = createInput(inputHolder, "100", tostring(config.AutoReturnDistance))

createLabel(inputHolder, "Height offset", UDim2.new(1, 0, 0, 16), Color3.fromRGB(255, 235, 235), true)
local offsetBox = createInput(inputHolder, "4", tostring(config.TeleportHeightOffset))

createLabel(inputHolder, "Tween time", UDim2.new(1, 0, 0, 16), Color3.fromRGB(255, 235, 235), true)
...
settingsButtons.Position = UDim2.new(0, 0, 0, 212)
...
local tabButtons = {
	Home = {button = tabHome, indicator = indHome},
	Teleport = {button = tabTeleport, indicator = indTeleport},
	Settings = {button = tabSettings, indicator = indSettings},
}
...
local function setPage(name)
	...
	if current then
		...
		TweenService:Create(current, TweenInfo.new(0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			Position = UDim2.new(-1, -18, 0, 0)
		}):Play()

		local tweenIn = TweenService:Create(nextPage, TweenInfo.new(0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
...
local function setUiVisible(visible)
	...
	pages.Home.Visible = visible and state.currentPage == "Home"
	pages.Teleport.Visible = visible and state.currentPage == "Teleport"
	pages.Settings.Visible = visible and state.currentPage == "Settings"
...
RunService.Heartbeat:Connect(function(dt)
	...
	if not state.teleportActive then return end
	if state.teleportInProgress then return end
	if now() < state.guardCooldownUntil then return end
...
local function createTab(parent, iconText, labelText, order)
	...
	innerPad.PaddingLeft = UDim.new(0, 18)
	innerPad.Parent = tab
...
local mainGrad = makeGradient(main, 0, Color3.fromRGB(35, 12, 12), Color3.fromRGB(14, 14, 16))
...
local railGrad = makeGradient(rail, 90, Color3.fromRGB(26, 14, 14), Color3.fromRGB(18, 18, 22))
...
local function createPagePadding(page)
	local pad = Instance.new("UIPadding")
	pad.PaddingTop = UDim.new(0, 2)
	pad.PaddingLeft = UDim.new(0, 2)
	pad.PaddingRight = UDim.new(0, 2)
...
createPagePadding(homePage)
createPagePadding(teleportPage)
createPagePadding(settingsPage)

--====================================================
-- HOME PAGE
--====================================================
local homeHero, homeHeroTitle, homeHeroDesc = createCard(
	homePage,
	108,
	"Control Hub",
	"Polished teleport control with auto-return protection, smooth motion, and an adjustable custom target part."
)
...
homeButtonsLayout.FillDirection = Enum.FillDirection.Horizontal
homeButtonsLayout.Padding = UDim.new(0, 10)
homeButtonsLayout.SortOrder = Enum.SortOrder.LayoutOrder
homeButtonsLayout.Parent = homeButtons
...
local statLeftTitle = createLabel(statLeft, "Target Part", UDim2.new(1, 0, 0, 18), Color3.fromRGB(255, 235, 235), true)
local statLeftValue = createLabel(statLeft, "Loading...", UDim2.new(1, 0, 0, 18), Color3.fromRGB(220, 220, 224), false)
statLeftValue.Position = UDim2.new(0, 0, 0, 26)
statLeftValue.TextSize = 11

local statRightTitle = createLabel(statRight, "Auto-Return", UDim2.new(1, 0, 0, 18), Color3.fromRGB(255, 235, 235), true)
local statRightValue = createLabel(statRight, "Enabled", UDim2.new(1, 0, 0, 18), Color3.fromRGB(220, 220, 224), false)
statRightValue.Position = UDim2.new(0, 0, 0, 26)
statRightValue.TextSize = 11
...
-- TELEPORT PAGE
--====================================================
local teleCard, teleTitle, teleDesc = createCard(
	teleportPage,
	126,
	"Smooth Teleport",
	"Moves your character to the assigned part with a tweened pivot path for a cleaner visual effect."
)
...
local guardInfo = createCard(
	teleportPage,
	80,
	"Guard Readout",
	"Waiting for a valid teleport and nearby location check."
)
...
local targetBoxLabel = createLabel(inputHolder, "Target part name", UDim2.new(1, 0, 0, 16), Color3.fromRGB(255, 235, 235), true)
local targetBox = createInput(inputHolder, "TeleportPart", config.TargetPartName)

local distanceBoxLabel = createLabel(inputHolder, "Auto-return distance", UDim2.new(1, 0, 0, 16), Color3.fromRGB(255, 235, 235), true)
local distanceBox = createInput(inputHolder, "100", tostring(config.AutoReturnDistance))

local offsetBoxLabel = createLabel(inputHolder, "Height offset", UDim2.new(1, 0, 0, 16), Color3.fromRGB(255, 235, 235), true)
local offsetBox = createInput(inputHolder, "4", tostring(config.TeleportHeightOffset))

local tweenBoxLabel = createLabel(inputHolder, "Tween time", UDim2.new(1, 0, 0, 16), Color3.fromRGB(255, 235, 235), true)
local tweenBox = createInput(inputHolder, "1.15", tostring(config.TeleportTweenTime))
...
settingsButtonsLayout.FillDirection = Enum.FillDirection.Horizontal
settingsButtonsLayout.Padding = UDim.new(0, 10)
settingsButtonsLayout.SortOrder = Enum.SortOrder.LayoutOrder
settingsButtonsLayout.Parent = settingsButtons
...
local settingsFoot = createCard(
	settingsPage,
	72,
	"Live Readout",
	"These settings affect both the teleport button and the auto-return guard."
)
...
-- TAB RAIL
--====================================================
local tabHome, indHome = createTab(rail, "⌂", "Home", 1)
local tabTeleport, indTeleport = createTab(rail, "↯", "Teleport", 2)
local tabSettings, indSettings = createTab(rail, "⚙", "Settings", 3)
...
local function updateTabs()
	for name, data in pairs(tabButtons) do
		...
		TweenService:Create(btn, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			TextColor3 = active and Color3.fromRGB(255, 245, 245) or Color3.fromRGB(230, 230, 234)
		}):Play()
	end
end
...
local function setPage(name)
	...
	if current then
		...
		local outTween = TweenService:Create(current, TweenInfo.new(0.26, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			Position = UDim2.new(-1, -18, 0, 0)
		})
		local inTween = TweenService:Create(nextPage, TweenInfo.new(0.26, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			Position = UDim2.new(0, 0, 0, 0)
		})
...
local function resetDefaults()
	config.TargetPartName = "TeleportPart"
	config.AutoReturnDistance = 100
	config.TeleportHeightOffset = 4
	config.TeleportTweenTime = 1.15
	config.AutoReturnEnabled = true
...
--====================================================
local function setUiVisible(visible)
	local targetMainSize = visible and UDim2.new(0, 620, 0, 390) or UDim2.new(0, 620, 0, 46)
	local targetShadowSize = visible and UDim2.new(0, 632, 0, 402) or UDim2.new(0, 632, 0, 58)
	local targetBackdrop = visible and 0.56 or 1
...
rescanBtn.MouseButton1Click:Connect(function()
	refreshReadouts()
	setTopStatus("RESCANNED", Color3.fromRGB(120, 220, 120))
end)
toggleButton.MouseButton1Click:Connect(toggleAutoReturn)
applyBtn.MouseButton1Click:Connect(applySettingsFromInputs)
resetBtn.MouseButton1Click:Connect(resetDefaults)
